  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-5">
      <div className="space-y-1 max-w-3xl mx-auto">
        ...
        {/* Active streaming indicator (when no text yet) */}
        {sending && streamingPhase !== 'streaming' && (
          <div className="group/msg">
            ...
            <div className="pl-4 sm:pl-5">
              {streamingPhase === 'tools' && activeTools.length > 0 ? (
                <div className="space-y-0.5">
                  {activeTools.map((tool, i) => (
                    <div key={i} className="flex items-center gap-2 py-1 px-2.5 rounded-md bg-surface-50 dark:bg-surface-800/40 border border-surface-200/50 dark:border-surface-700/30">
                      <div className="w-1 h-1 rounded-full bg-amber-500 animate-pulse" />
                      <span className="text-xs font-mono text-surface-600 dark:text-surface-400">{tool}</span>
                      <svg className="w-3 h-3 text-surface-400 animate-spin ml-auto" viewBox="0 0 24 24" fill="none">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                      </svg>
                    </div>
