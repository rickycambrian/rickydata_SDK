...
describe('mcp commands', () => {
  ...
  describe('mcp connect', () => {
    it('executes claude mcp add or prints command with token when authenticated', async () => {
      ...
      expect(
        output.includes('rickydata MCP server added to Claude Code') ||
    ...
    it('prints command in dry-run mode with token', async () => {
      ...
      expect(output).toContain('rickydata');
      expect(output).toContain('https://rickydata-mcp-server-2dbp4scmrq-uc.a.run.app/mcp');
    ...
    it('prints unauthenticated command in dry-run mode when not logged in', async () => {
      ...
      const output = consoleSpy.mock.calls.map((c) => c.join(' ')).join('\n');
      expect(output).toContain('Not authenticated. Opening browser to log in');
      expect(output).toContain('After authenticating, run: rickydata auth login');
      expect(output).toContain('Then re-run: rickydata mcp connect');
