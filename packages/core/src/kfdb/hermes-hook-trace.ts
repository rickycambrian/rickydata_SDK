...
export function buildHermesHookTraceOperations(trace: HermesHookTrace): Array<Record<string, unknown>> {
  ...
  const chatSessionNodeId = deterministicId('AgentChatSession', [wallet, trace.agentId, trace.sessionId]);
  const chatTurnNodeId = deterministicId('AgentChatTurn', [wallet, trace.agentId, trace.sessionId, trace.turnIndex]);
  ...
  const operations: Array<Record<string, unknown>> = [
    ...
    { operation: 'create_node', id: chatSessionNodeId, label: 'AgentChatSession', mode: 'merge', properties: { agent_id: value(trace.agentId), session_id: value(trace.sessionId), external_session_id: value(trace.sessionId), wallet_address: value(wallet), source: value('hermes-hooks'), privacy_scope: value('private'), schema_version: value(TRACE_SCHEMA_VERSION), updated_at: value(trace.completedAt) } },
    { operation: 'create_node', id: chatTurnNodeId, label: 'AgentChatTurn', mode: 'merge', properties: { agent_id: value(trace.agentId), session_id: value(trace.sessionId), turn_index: value(trace.turnIndex), model: value(model), provider: value(provider), execution_engine: value('hermes'), cwd: value(trace.cwd ?? ''), source: value('hermes-hooks'), privacy_scope: value('private'), schema_version: value(TRACE_SCHEMA_VERSION), started_at: value(trace.startedAt), completed_at: value(trace.completedAt) } },
    ...
    { operation: 'create_edge', id: deterministicId('HAS_TURN', [sessionNodeId, turnNodeId]), from: sessionNodeId, to: turnNodeId, edge_type: 'HAS_TURN', properties: { turn_index: value(trace.turnIndex) } },
    { operation: 'create_edge', id: deterministicId('MIRRORS_AGENT_CHAT_SESSION', [sessionNodeId, chatSessionNodeId]), from: sessionNodeId, to: chatSessionNodeId, edge_type: 'MIRRORS_AGENT_CHAT_SESSION', properties: { source: value('hermes-hooks') } },
    { operation: 'create_edge', id: deterministicId('MIRRORS_AGENT_CHAT_TURN', [turnNodeId, chatTurnNodeId]), from: turnNodeId, to: chatTurnNodeId, edge_type: 'MIRRORS_AGENT_CHAT_TURN', properties: { source: value('hermes-hooks') } },
