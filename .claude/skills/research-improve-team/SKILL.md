...
This creates the team AND a shared task list at `~/.claude/tasks/research-improve-team/`.
...
TaskCreate(
  ...
  description: "Thoroughly explore the repository. Read CLAUDE.md, scan for TODOs/FIXMEs, assess test coverage, map module structure. Focus on: $ARGUMENTS. Produce a structured gap report with architecture summary, identified gaps (file:line references), technical debt, and top 3-5 improvement areas ranked by impact. When done, send the FULL report to team lead via SendMessage."
)
...
TaskCreate(
  ...
  description: "Wait for the full pipeline output from the team lead. Study the Claude Code skills docs (https://code.claude.com/docs/en/skills) and sub-agents docs (https://code.claude.com/docs/en/sub-agents). Analyze the full pipeline output for reusable patterns: successful tool sequences, effective queries, team compositions that worked. Only record patterns that were VERIFIED WORKING in this session. Create or update skills in .claude/skills/ and agents in .claude/agents/. Update CLAUDE.md with references. Send summary of what was recorded to team lead via SendMessage."
)
...
Agent(
  ...
  prompt: "You are the 'documenter' teammate on the 'research-improve-team'. Your task is blocked until the synthesizer finishes. Wait for the full pipeline output from the team lead. First study Claude Code docs on skills and sub-agents. Then analyze the pipeline output for reusable patterns. CRITICAL: Only record patterns that were VERIFIED WORKING in this session. Do NOT hallucinate or record patterns you haven't confirmed. Create skills in .claude/skills/, update agents in .claude/agents/, and add references to CLAUDE.md. Send summary to team lead via SendMessage."
)
```
...
- `rickydata-proxy` MCP server connected: `claude mcp list | grep rickydata-proxy`
